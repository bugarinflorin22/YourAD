    ...
    && !empty($nume) && !empty($discord) && !empty($select1) && !empty($select2) && !empty($select3) && !empty($pret)) {
        ...
        if ($lungime3 <= 5) {
                ...
                if ($da != 0) {
                    ...
                    ('$nume', '$select1', '$discord', '$datafinala', '$pret', '$avere2', '$type', $premium)";
                    $result = mysqli_query($conn, $sql);
                    ...
                    $sql = "INSERT INTO ad (nume, server, discord, data, pret, ofera, type, premium) VALUES
                    ('$nume', '$select1', '$discord', '$datafinala', '$pret', '$select3', '$type', $premium)";
                    $result = mysqli_query($conn, $sql);
                    $sql = "SELECT * FROM reg WHERE nume='$nume'";
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_array($result);
                    ...
                    if ($manevraa == 0){
                    $sql = "INSERT INTO ad (nume, server, discord, data, pret, ofera, type, premium) VALUES
                    ('$nume', '$select1', '$discord', '$datafinala', '$pret', '$altceva', '$type', '$premium')";
                     $result = mysqli_query($conn, $sql);
                     $sql = "SELECT * FROM reg WHERE nume='$nume'";
                     $result = mysqli_query($conn, $sql);
                     $row = mysqli_fetch_array($result);
                    ...
                    ('$nume', '$select1', '$discord', '$datafinala', '$pret', '$altceva', '$manevra', '$type', '$premium')";
                     $result = mysqli_query($conn, $sql);
                     $sql = "SELECT * FROM reg WHERE nume='$nume'";
                     $result = mysqli_query($conn, $sql);
                     $row = mysqli_fetch_array($result);
